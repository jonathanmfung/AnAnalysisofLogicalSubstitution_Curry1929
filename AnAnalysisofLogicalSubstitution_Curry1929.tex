...
\linespread{1.1}% Original is slightly more lineheight
...
% space after footnote marker in bottom margin https://tex.stackexchange.com/a/54691
...
\begin{abstract}
  \begin{itshape}
    ...
    This document is a digital recreation in \LaTeX{}, source repo is at \href{https://github.com/jonathanmfung/AnAnalysisofLogicalSubstitution_Curry1929}{github.com/jonathanmfung/AnAnalysisofLogicalSubstitution\_Curry1929}.
...
methods of mathematics to the domain of Logic.\footnote{Hilbert, D., and Ackermann, W., \textit{Grundzüge der theoretischen Logik}, 1928, p. 1.} Logic, on the other hand,
is the analysis and criticism of thought.\footnote{Johnson, W. E., \textit{Logic}, Part I, Cambridge (1921), p. xiii.} In accordance with these definitions, the essential purpose of mathematical logic is the construction of an
...
been abstracted. This requires that the sense of the term meaning be explained. If we take the term meaning, as applied to objects, to signify the
...
deduction of the theory depends; accidental meanings those which are nonessential. The distinction between these three kinds of meaning is important
in what follows.
...
intention, definition, or the like. The former corresponds to an act of judgment,the latter to one of volition. Common sense and grammar have long
recognized both of these types; yet logicians seem to belittle the latter in that they define the proposition so as to exclude it.\footnote{See Johnson, W. E., \textit{1. c.}, p. 1. Johnson's definition of the proposition is what I have given as the definition of a statement of fact.} Both these kinds of statement, however, are equally intelligible to a rational mind; in this sense it is
...
attached, although they need not coincide with any ideas previously entertained.\footnote{\textit{I. e.}, their meaning may be partly conventional.} For example:
...
are combinations. It must be specified by rules that the results of combination are entities. In the simple cases only one such notion is necessary, and
...
\section{Rules.}
\indent Statements of the processes by means of which new entities\footnote{Strictly speaking we should consider in the theory not only statements that an entity is an assertion, but also statements that such and such combinations are entities. But the latter are, in simple cases at least, of so trivial a nature that it is not necessary to give them special prominence.} or new
...
theory may be communicated. The use of this language may involve intuitive operations other than those we have mentioned; it is desirable that these,
...
that it is identical with some already existing entity. It is worth emphasizing that since statements that entities are not assertions do not occur among
...
assumed is the appreciation of the essential meanings and conventional statements appearing in the primitive frame. When the theory is interpreted the
additional knowledge that must be brought to bear consists of the following:
that the concepts which we substitute for the primitive ideas have the necessary essential meanings, and that the conventional statements in the primitive
...
used in its expression, and to the various concrete theories obtained by interpreting it, it suffices to say that such questions are largely metaphysical, and
...
exclusion of all others; any more than it is desirable that two persons following the same argument should have the same mental imagery.\footnote{In writing the foregoing account I have naturally made use of any ideas I may have gleaned from reading the literature. The writings of Hilbert are fundamental in this connection. I hope that I have added clearness to certain points where the existing treatments are obscure.}

The next point to which I wish to direct the reader's attention is the cardinal importance of the rules in any abstract theory related to logic. For the
amount of initial knowledge which enters into the first three categories of the
...
the contemplated act is or is not according to Hoyle. These judgments, moreover, are the only ones which are required. The rules, therefore, form the
...
this general problem, I shall discuss in the rest of this paper a special one connected with it; viz., the analysis of the process of substitution. The latter
...
With each theory there is associated a metatheory in which we reason intuitively about the theory. In this metatheory we can derive more and more
complicated rules by showing, in general terms, how any particular consequence of the derived rules can actually be deduced from the primitive ones.
...
entity for one or more of the variables in a propositional function. The complexity of this process is manifest. For not only is a function of \(n\) variables
a distinct concept for every value of \(n\), but the constant may be inserted in
any one of the \(n\) places, and each such insertion is a distinct act; furthermore, in connection with the universal and existential prefixes,we have a
process which virtually amounts to the simultaneous substitution of an entity
in two or more distinct places, as a procedure distinct from any of the foregoing. The process of substitution is therefore compound in that it is not
one maneuver, but many. Moreover, when these acts of substitution are performed in succession, there are many equivalences between the different possibilities. To take the simplest example: suppose in a given function \(\phi(x, y)\)
...
A notion closely related to substitution is that of transformation of functions. Suppose we regard a function as having inherent in its definition a
...
the old; let us call them transforms of the original function, and the operations by which they are produced transformations. If we number the variables consecutively \(1, 2, 3, \cdots,\) then the transforms for a function of two
variables will be---
\[\phi(1, 2),\, \phi(2, 1),\, \phi(1, 1).\]
For three variables there will be 13 transforms, for four variables 75, for five
variables 541, etc. It is clear that the process of substituting a series of constants in an arbitrary manner (such that the total number of entities counting
...
M. Schönfinkel.\footnote{``Ueber die Bausteine der mathematischen Logik,'' \textit{Mathematische Annalen}, Vol. 92 (1924), pp. 305-316.} Starting, apparently, from the fact that every logical formula is a combination of constants---the variables being only apparent---he
...
\section{Formal Primitive Ideas.}
\indent Three, denoted by \((\longeq)\), \(S\) and \(K\). In the interpretation \((\longeq)\) is to correspond with identity; \(S\) and \(K\) are operations in the sense defined by the
rules.
...
\item The outside parentheses may be left off in the case of a symbol standing by itself or on either side of the sign \(\longeq\).
\end{enumerate}
...
\item \label{rule:schonfinkel0}If \(x\) and \(y\) are entities, then \((xy)\) shall be an entity.
...
\item If \(x,y,z\) are entities, then
  \[Sxyz \longeq xz(yz)\]
\item \label{rule:schonfinkel4}If \(X\) and \(Y\) are combinations of \(S\) and \(K\), and if there exists an
...
Let \(x_1, x_2,\cdots, x_n\) be any \(n\) entities, and \(X\) any combination of them constructed by means of application. Then there exists a unique \(Y\), which is a
combination of \(S\) and \(K\) and independent of \(x_1, x_2,\cdots, x_n\) such that
\[X \longeq Y x_1 x_2 \cdots x_n.\]

When we recall the interpretation to be given to application we have the following result: given any logical formula built up from functions \(f_1, f_2,\cdots, f_m\)
and variables \(x_1, x_2,\cdots, x_n\) by substitution and rearrangement in any manner; then the formula is expressible in the form
\[Fx_1x_2\cdots x_m\]
where
...
of the complexity of \hyperref[rule:schonfinkel4]{Rule 4}. This rule is not mentioned by Schönfinkel; but
it is necessary in order that the \(Y\) mentioned be unique. In fact, the combinations \(SK\) and \(K(SKK)\) determine the same \(X\); yet it is evidently not possible to establish their identity by means of the first four rules.

This situation suggests a problem; viz., to find a set of postulates which,
when adjoined to the Schönfinkel frame, enable us to dispense with \hyperref[rule:schonfinkel4]{Rule 4}.
In what follows we shall obtain the solution of a special case under this problem; specifically, we shall find a set of postulates such that, within a certain
subclass of combinations of \(S\) and \(K\), all the \(Y\)'s which correspond
to the same \(X\) may be proved equal by means of these postulates and \hyperref[rule:schonfinkel0]{Rules 0-3}. The subclass is one which has particular reference to logical substitution.
...
some conventions in regard to transformations. Let us redefine a transformation as an operation converting the sequence \(1, 2, 3, \cdots\) into the sequence
...
integer having this property; then we shall call \(m\) the order of the transformation. We shall denote a transformation by writing in brackets the
...
a function of any number of variables \(\geqq m\); and that the effect of the transformation on the function \(\phi(1, 2, \cdots, n)\), where \(n \geqq m\), shall be the transform \(\phi(a_1, a_2, \cdots a_n)\). We shall regard as undefined the effect of a transformation on a function of multiplicity less than the order of the transformation. Then there is one and only one transformation which carries a given
...
I now assert that every entity of our subclass corresponds to a transformation; and conversely that every \(X\) corresponding to a transformation, is an
...
between our subclass and the set of all transformations is a simple isomorphism. To this we now turn.

\newpage
\fakepart{Solution of the Combinatory Problem.}
\label{part:III}
\begin{theorem}
  \begin{enumerate}[label=\arabic*),leftmargin=0pt,itemindent=3em]
\item[]% newline after start of theorem https://tex.stackexchange.com/a/37806
\item Let \(\mathfrak{A}\) be a system of operators, in which there exists an associative
    multiplication and an identical element, \(I\).
\item Let this system be generated by operators \(W, C_1, C_2, \cdots\) Subject to the postulates
  \label{postulates}
  \begin{alignat*}{3}
    \textup{I.}\quad& C_i \cdot C_i \longeq I &&i \longeq 1, 2, 3, \cdots\\
    \textup{II.}\quad& C_i \cdot C_{i+1} \cdot C_i \longeq C_{i+1} \cdot C_i \cdot C_{i+1} &&i \longeq 1, 2, 3, \cdots\\
    \textup{III.}\quad& C_i \cdot C_j \longeq C_j \cdot C_i &&i \longeq 1, 2, 3, \cdots \quad j > i + 1 \\
    \textup{IV.}\quad& C_i \cdot W \longeq W \cdot C_{i+1} &&i \longeq 2, 3, 4, \cdots \\
    \textup{V.}\quad& W \cdot C_1 \longeq W \\
    \textup{VI.}\quad& W \cdot W \cdot C_2 \longeq W \cdot W\footnotemark \\
    \textup{VII.}\quad& W \cdot D_j \cdot W \longeq D_{j-1} \cdot W \cdot D_3 \cdot W \cdot C_2 \: \cdot && \: C_3 \cdot C_1 \cdot C_2   \addtocounter{footnote}{-1}\footnotemark\qquad j \longeq 3, 4, 5 \cdots
  \end{alignat*}
  % https://tex.stackexchange.com/a/21817
  \footnotetext{VI and VII are equivalent respectively to \eqref{eq:6} and \eqref{eq:7} (see below). The latter may if desired replace VI and VII.}
  \textup{where}
  \begin{align*}
    &D_1 \longeq I \\
...
\item Let a correspondence be set up between the system \(\mathfrak{A}\) and the system of transformations of finite order as follows:
  \begin{align}
    W &\longsim [1, 1] \nonumber \\
    C_1 &\longsim [2, 1] \label{eq:finite_transformations} \\
    C_2 &\longsim [1, 3, 2] \nonumber \\
    C_i &\longsim [1, 2, 3, \cdots i - 1, i + 1, i] \nonumber
  \end{align}
  and if \(A \longsim \alpha\), \(B \longsim \beta\)\footnote{Throughout this discussion we use Roman capitals to denote operators of \(\mathfrak{A}\), Greek l. c. letters to denote transformations.} then \(A \cdot B \longsim \alpha \cdot \beta\) is defined by \eqref{eq:multiplication_transformations}.
  \end{enumerate}
  \noindent Then, the correspondence so defined is a one-to-one isomorphism.
\end{theorem}
\begin{proofnewlineindent}
  ...
  \item That if two transformations correspond to two expressions in \(\mathfrak{A}\) which
    are equal by Virtue of \hyperref[postulates]{I-VII} (inclusive), then these transformations are equal.
  \item That conversely there corresponds an operator of \(\mathfrak{A}\) to each transformation, and any two operators corresponding to the same transformation may
    be proved equal by \hyperref[postulates]{I-VII}.
  \end{enumerate}

  Of these four things the first three follow immediately: the first two
  by definitions; the third because propositions analogous to \hyperref[postulates]{I-VII} are true
...
often designate operators composed of \(C\)'s only by means of the transformations corresponding to them.
...
form \(W_k \cdot B\), where \(A\) and \(B\) involve the \(C\)'s only. If \(A\) is the identical element we are through. If not, consider the \(C\) nearest the \(W\); if this is any
...
\noindent (the first and third relations follow from the known properties of permutations, the second from IV.) Combining these three we have \eqref{eq:8}.
...
\noindent \textit{Sixth,}
\newline \indent For \(j\longeq 1\), this is true by \eqref{eq:6}. Suppose it true for \(j-1\), then
\begin{flalign*}
  && W_j \cdot W_j &\longeq C_{j-1} \cdot W_{j-1} \cdot C_j \cdot W_{j-1} \cdot C_j \cdot C_{j-1} & \text{by def. and I.} \\
                 ...
                 &&&\longeq C_{j-1} \cdot W_{j-1} \cdot C_j \cdot C_{j-1} \cdot C_{j-1} \cdot C_j & \mkern-50mu\cdot\: W_j \cdot C_{j+1} \cdot C_j \cdot C_{j-1} \quad \text{by I.} \\
                 &&&\longeq W_j \cdot C_{j-1} \cdot W_{j+1} \cdot C_{j-1} & \text{by def.} \\
                 &&&\longeq W_j \cdot W_{j+1} &\text{q.e.d. \qquad by \eqref{eq:8} and I.}
\end{flalign*}

\noindent \textit{Seventh,}
\[W_j \cdot W_{k+1} \longeq W_k \cdot W_j \qquad \text{for } k > j\]
\indent For \(j\longeq 1\), this is true by \eqref{eq:7}. Suppose it true for \(j-1\), then,
\begin{flalign*}
  && W_k \cdot W_j &\longeq W_k \cdot C_{j-1} \cdot W_{j-1} \cdot C_j \cdot C_{j-1} & \text{by def.} \\
                 &&&\longeq C_{j-1} \cdot W_k \cdot W_{j-1} \cdot C_j \cdot C_{j-1} & \text{by \eqref{eq:8}, since } j-1<k-1 \\
                 &&&\longeq C_{j-1} \cdot W_{j-1} \cdot W_{k+1} \cdot C_j \cdot C_{j-1} & \text{by hyp.} \\
                 &&&\longeq C_{j-1} \cdot W_{j-1} \cdot C_j \cdot C_{j-1} & \text{by \eqref{eq:8}} \\
                 &&&\longeq W_j \cdot W_{k+1} & \text{q.e.d. \qquad by def.}
\end{flalign*}
...
\end{lemma}
\begin{proofnewlineindent}
  \indent Let \[\Delta_q \longeq W_{k_q}^{r_q} \cdot W_{k_{q-1}}^{r_{q-1}} \cdots W_{k_2}^{r_2} \cdot W_{k_1}^{r_1}\]
  \setlength{\parindent}{0pt}
  also let \[\Delta_q \longsim \alpha \longeq [a_1, a_2, a_3 \cdots \cdots]\]
...
\begin{proofnewlineindent}
  ...
  we may suppose that \(i_1 < i_2 < \cdots < i_p\). Then \(\alpha \cdot \beta \longeq \gamma\); for the i'th integer in the symbol for \(\alpha \cdot \beta\) is \(a_{b_i}\), and \(b_i\) has been so chosen that in all
...
\end{lemma}
\begin{proofnewlineindent}
  Let \(A_1\) and \(A_2\) be two operators in \(\mathfrak{A}\) which correspond to a given transformation \(\gamma\). By \Cref{lemma:2,lemma:3} there is a uniquely determined \(\Delta_q\)
  such that
  ...
  to which \(\beta\) was subject in \Cref{lemma:4}, except that it is not necessary to suppose
  ...
  where \(\beta_3\) is a product of permutations, each of which permutes among them  selves the integers composing one of the sets \(k_j + s_{j-1}, k_j + s_{j-1} + 1, \cdots k_j + s_j\).
  ...
  This follows from \eqref{eq:10}, under \Cref{lemma:2}.
  ...
  For \(j\longeq 0\), this is true by \eqref{eq:9}, under \Cref{lemma:2}.
  For \(j\longeq 1\)
  \begin{flalign*}
...
\end{proofnewlineindent}% lemma
\end{proofnewlineindent}% theorem
We have now achieved one of our objectives---viz. the elimination of
\hyperref[rule:schonfinkel4]{Rule 4} so far as the selected subclass is concerned. For Rule 4 merely
enables us to conclude that two entities of the subclass, whose application to
a series \(x_0 x_1 x_2 \cdots\) yields the same transformation, are equal. These entities correspond to the same transformation in the sense of our theorem,
...
in terms of \hyperref[postulates]{I-VII}, together with the rules for identity and no more. This
...
There remain open two questions: \begin{enumerate*}[label=\arabic*)] \item What is the relation between the
  transforms of a function of \(n - 1\) variables, obtained by substituting a constant in a function of \(n\) variables, and the transforms of the original;
  \item How can our infinite set of postulates be reduced to a finite set. \end{enumerate*} Both
these questions can be answered by introducing the entity \(B\) defined in \Cref{part:II}.
