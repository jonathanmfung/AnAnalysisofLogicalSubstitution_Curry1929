...
\item Let this system be generated by operators \(W, C_1, C_2, \cdots\) Subject to the postulates
  ...
  \footnotetext{VI and VII are equivalent repectively to \eqref{eq:6} and \eqref{eq:7} (see below). The latter may if desired replace VI and VII.}
  \textup{where}
  \begin{align*}
    &D_1 \longeq I \\
...
\begin{proofnewlineindent}
  ...
  \item That conversely there corresponds an operator of \(\mathfrak{A}\) to each transfor    mation, and any two operators corresponding to the same transformation may
...
\noindent (the first and third relations follow from the known properties of permuttions, the second from IV.) Combining these three we have \eqref{eq:8}.
...
\begin{proofnewlineindent}
  ...
  we may suppose that \(i_1 < i_2 < \cdots < i_p\). Then \(\alpha \cdot \beta \longeq \gamma\); for the i'th in  teger in the symbol for \(\alpha \cdot \beta\) is \(a_{b_i}\), and \(b_i\) has been so chosen that in all
...
\end{lemma}
\begin{proofnewlineindent}
  Let \(A_1\) and \(A_2\) be two operators in \(\mathfrak{A}\) which correspond to a given trans  formation \(\gamma\). By \Cref{lemma:2,lemma:3} there is a uniquely determined \(\Delta_q\)
  such that
